...
// webclaw addition: direct browser emulation config without Any downcast
#[cfg(feature = "__rustls")]
impl ClientBuilder {
    /// Set a pre-built rustls ClientConfig directly.
    /// Unlike tls_backend_preconfigured, this doesn't use Any downcasting.
    pub fn rustls_config(mut self, config: rustls::ClientConfig) -> ClientBuilder {
        self.config.tls = crate::tls::TlsBackend::BuiltRustls(Box::new(config));
        self
    }
}
