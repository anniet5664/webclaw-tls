...
fn emit_client_hello_for_retry(
    ...
    if let Some(be) = config.browser_emulation.as_ref() {
        match be.browser_type {
            ...
            BrowserType::Safari => {
                // Safari fingerprint captured from real Safari 18 on macOS via tls.peet.ws
                // JA4: t13d2013h2_a09f3c656075_7f0f34a4126d
                // Safari DOES use GREASE (first + last), has a specific non-ascending order.
                // No session_ticket, no ECH, no ALPS, no delegated_credentials, no PSK.

                exts.signed_certificate_timestamp = Some(());
                exts.renegotiation_info = Some(PayloadU8::empty());

                // GREASE extensions for Safari (same pattern as Chrome: first + last)
                exts.unknown_extensions
                    .push((ExtensionType::Unknown(0x6a6a), Payload::empty()));
                exts.unknown_extensions
                    .push((ExtensionType::Unknown(0x7a7a), Payload::empty()));

                // Real Safari extension order (captured from tls.peet.ws):
                // GREASE, SNI, EMS, RenegotiationInfo, Groups, ECPoints, ALPN,
                // StatusRequest, SigAlgs, SCT, KeyShare, PSKModes, Versions,
                // CompressCert, GREASE
                exts.contiguous_extensions = vec![
                    ExtensionType::Unknown(0x6a6a),       // GREASE (first, placeholder)
    ...
    if let Some(be) = config.browser_emulation.as_ref() {
        if matches!(be.browser_type, BrowserType::Chrome | BrowserType::Edge) {
            // Chrome 146 extension order — captured from real Chrome via tls.peet.ws
            // GREASE, extended_master_secret, ALPN, supported_groups, supported_versions,
            // key_share, server_name, ec_point_formats, signature_algorithms, ECH,
            // compress_certificate, session_ticket, ALPS, status_request,
            // psk_key_exchange_modes, renegotiation_info, SCT, GREASE, pre_shared_key
            exts.contiguous_extensions = vec![
                ExtensionType::Unknown(0x6a6a),       // GREASE (first, placeholder)
                ExtensionType::ExtendedMasterSecret,  // 23 (0x0017)
                ExtensionType::ALProtocolNegotiation, // 16 (0x0010) ALPN
                ExtensionType::EllipticCurves,        // 10 (0x000a) supported_groups
                ExtensionType::SupportedVersions,     // 43 (0x002b)
                ExtensionType::KeyShare,              // 51 (0x0033)
                ExtensionType::ServerName,            // 0 (0x0000)
                ...
                ExtensionType::Unknown(0x44cd),       // ALPS (17613)
                ExtensionType::StatusRequest,         // 5 (0x0005)
                ExtensionType::PSKKeyExchangeModes,   // 45 (0x002d)
                ExtensionType::RenegotiationInfo,     // 65281 (0xff01)
                ExtensionType::SCT,                   // 18 (0x0012)
                ExtensionType::Unknown(0x0a0a),       // GREASE (last, placeholder)
                                                      // pre_shared_key (41) is added separately after all extensions (line ~1111)
                                                      // as required by RFC 8446 — it must be the absolute last extension
            ];
    ...
    if let Some((_, _, grease_ext_first, grease_ext_last)) = grease_vals {
        ...
        // Update unknown_extensions GREASE entries to match contiguous_extensions
        for (ext_type, _) in exts.unknown_extensions.iter_mut() {
            ...
            } else if *ext_type == ExtensionType::Unknown(0x7a7a) {
                *ext_type = ExtensionType::Unknown(grease_ext_last);
            }
        }
    }

    // For Chrome/Edge/Opera emulation: add a dummy PSK offer so pre_shared_key (41)
    // appears in the ClientHello. Chrome always sends PSK even on fresh connections.
    // Firefox and Safari do NOT send PSK on first connections.
    // The server won't recognize the random identity and will do a full handshake.
    #[cfg(feature = "impersonate")]
    if config.browser_emulation.as_ref().is_some_and(|be| {
        ...
        && exts.preshared_key_offer.is_none()
    {
        // Derive PSK identity from existing client random — zero extra RNG calls.
        // 252 bytes (Chrome's typical session ticket size) derived via XOR mixing.
        let rand = &input.random.0;
        let mut dummy_identity = Vec::with_capacity(252);
        for i in 0..252 {
            dummy_identity.push(rand[i % 32] ^ (i as u8).wrapping_mul(0x9e));
        }
        let dummy_age = u32::from_be_bytes([rand[28], rand[29], rand[30], rand[31]]);
        // 32-byte binder derived from client random (SHA-256 output size)
        let dummy_binder: Vec<u8> = rand.iter().map(|b| b.wrapping_add(0x5a)).collect();

        exts.preshared_key_offer = Some(PresharedKeyOffer::new(
            PresharedKeyIdentity::new(dummy_identity, dummy_age),
            dummy_binder,
        ));
    }
