...
impl<'a> Codec<'a> for CertificateExtensions<'a> {
    ...
    fn read(r: &mut Reader<'a>) -> Result<Self, InvalidMessage> {
        ...
        while sub.any_left() {
            // Skip unknown certificate extensions (e.g. SCT) instead of erroring.
            // Chrome's ClientHello signals SCT support, so servers include it
            // in certificate entries. The extension body is already safely
            // consumed by r.rest() in the macro — just don't reject it.
